...
main (int argc, char *argv[]) {
	...
	int tcat=1, rcat=0, verb=0, miss=0;
	...
	data->r_update = -1;
	data->exact = 0;
	data->rates_filename = (char *) malloc((size_t) MAXFILENAME*sizeof(char));
	data->rates_filename[0]='\0';
	data->blocks_filename = (char *) malloc((size_t) MAXFILENAME*sizeof(char));
	data->blocks_filename[0]='\0';
	for(i = 0; i < argc; i++)
	{
		if(*argv[i] == '-')
		{ 
			...
			if(strcmp(in_str, "-tag") == 0) strncpy(data->rates_filename, argv[i+1], MAXFILENAME);
		}
	}
	...
	if (data->rates_filename[0]=='\0') {
		strcat(data->rates_filename, "rates.txt");
		strcat(data->blocks_filename, "bounds.txt");
	}
	else {
		strcpy(data->blocks_filename, data->rates_filename);
		strcat(data->rates_filename, "_rates.txt");
		strcat(data->blocks_filename, "_bounds.txt");
	}
...
/*Read sequences */	
	...
	allele_count(seqs, data->nseq, data->lseq, nall,1, data->hd);

	/*Store lnfac values in array for speed of computation*/
	lnfac_array = (float *) malloc((size_t) ((int) data->nseq*data->hd+2)*sizeof(float));
	...
	fscanf(ifp2, "%i %lf", &ns, &data->tlseq);
	data->lc = fgetc(ifp2);
	while((data->lc <33) || (data->lc>122)) data->lc = fgetc(ifp2);
	...
	for (i=1; i<=data->lseq; i++) {
		...
		if ((locs[i]<=0)||(locs[i]>data->tlseq)) {
			printf("Error in Loc file (locs[%i] = %.5lf, TL = %.5lf)\n\n", i, locs[i], data->tlseq);
			exit(1);
		}
...
struct data_sum *data;
{
  ...
  rfp = fopen(data->rates_filename,"w");
  bfp = fopen(data->blocks_filename,"w");
...
//	  check_blocks(blockp[0],data->rmap,loc);
  ...
  printf("\n\nAcceptance rates: \nRate change= %.4f\nExtend = %.4f\nSplit = %.4f\nMerge = %.4f\n\n",\
	  (float) nacc[0][1]/nacc[0][0], (float) nacc[1][1]/nacc[1][0], (float) nacc[2][1]/nacc[2][0], (float) nacc[3][1]/nacc[3][0]);
...
struct block ** update_blocks(pr,blockp,nblock,loc,lk,nacc,data,lkmat,pij,lij)
	 ...
	 double **lkmat, **lij, lk[2], *loc, pr[4];
	 struct data_sum *data;
{
...
//  rcheck = malloc((size_t) (data->lseq+1)*sizeof(float));

  /*Choose block and event for updating*/
  i = (int) ((float) (*nblock)*ran2());
  if (DEBUG) printf("\nUpdating block %i\n", i+1);
...
//				for (j=1;j<=data->lseq;j++) if (fabs(rcheck[i]-rmap[i])>0.01) {printf("\n\nError in rmap\n\n"); exit(1);}
  ...
  /*Split block - generate new block: NB reversible jump step*/
    ...
    blockp[j]=new_block;
    blockp[j]->num=j;
    split = (int) ((float) ran2()*(blockp[i]->size-1))+1;/*Choose size of LHS in terms of SNPs*/
...
//		exit(1);
	...
	rl = (double) maxf((float) RMIN,(float) (rt-RMAX*lr)/ll);/*define bounds for new rate - approx 0*/
	ru = (double) minf((float) RMAX,(float) (rt-RMIN*lr)/ll);
...
//	  jcb = (float) (ll+lr)/lr*(1-exp(-or*(ll+lr)/ll))/exp(-blockp[i]->rate); /*For exponential prior update*/
//	  jcb = (float) or*(ll+lr)*(ll+lr)/(ll*lr); /*Jacobian for linear choice*/
	  fl=1;
	}
     ...
     double *lkres,**lkmat, *lknew;
{
  ...
  for (i=0;i<=pset->pt[15];i++) {
    mn = (float) lnfac_array[pset->pt[15]]-lnfac_array[i]-lnfac_array[pset->pt[15]-i];
...
/*		  mn = (float) lognC2(pset->pt[4],a)+lognC2(pset->pt[5],b)+lognC2(pset->pt[6],c)+lognC2(pset->pt[7],d)+lognC4(pset->pt[8],e1,e2,e3,e4);*/
		  mn = (float) lnfac_array[pset->pt[4]]-lnfac_array[a]-lnfac_array[pset->pt[4]-a]+lnfac_array[pset->pt[5]]-lnfac_array[b]-lnfac_array[pset->pt[5]-b]+\
	...
	for(i = 0; i < argc; i++){
		if(*argv[i] == '-'){ 
			...
			if((strcmp(in_str, "-h") == 0) || (strcmp(in_str, "-?") == 0) || (strcmp(in_str, "-help") == 0)) {
				...
				printf(" -tag <file>               Prefix for output filenames\n");
				printf("\n\n");
				exit (0);
			}
		}
