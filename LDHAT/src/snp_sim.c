...
struct data_sum *data;
{
	...
	float rtot, stot, *rho_sim, *clr_sim, *vmin, *vmax;
	extern int n_node;
	...
	fprintf(ofp,"\nRho     Fit     CLR     CF\n\n");
	  ...
	  for (i=1;i<=data->lseq;i++) con.rmap[i] = (float) (locs[i]-1)*(data->rho)/(data->tlseq-1);
	  con.cond=1;
	...
	rho_sim = vector(1,nrun);
	clr_sim = vector(1,nrun);
	...
	free_vector(con.rmap,1,con.len);
	...
	free_vector(rho_sim,1,nrun);
	free_vector(clr_sim, 1, nrun);
	free_vector(vmin,1,n5);
	free_vector(vmax,1,n5);
}
...
int *revts, **segl;
{
	...
	for (i=1; i<=con->nsamp; i++) {
		...
		tree_ptr[i]->rlen=(float) con->rmap[con->len]-con->rmap[1];
		list[i] = tree_ptr[i];
	}
	...
	/*Choose type of next event: update list and tree accordingly*/
		...
		else if (ran2() < (float) avR/(avR+(k-1)*exp((con->lambda)*t))) {  /*Event is a recombination*/
			k++; nrec++; n_node+=2; 
			list = (struct node **) realloc(list, (size_t) (k+1)*sizeof(struct node *));
...
int nsamp, site, fsim, nnode;
{
	...
	int i, desc, j=0, fl;
	float r1, len=0.0, ct=0, **lnode;

	lnode = matrix(1,nnode,1,3);
	...
	for (i=1;i<=nnode; i++) {
	  if (tree[i]->asite[site]) {
		...
		lnode[i][1]=(float) tree[i]->node_num; 
		lnode[i][2]=((tree[i]->a[0])->time-tree[i]->time);
		lnode[i][3]=(float) desc;
...
struct control *con;
{
  ...
  float u=-log(ran2()), cons[3];

  if (!(con->growth)) { /*Constant population size*/
    (*t) += (float) 2*u/(k*rho+k*(k-1));
  }
  else { /*Exponential growth*/
    if (con->lambda == 0) {printf("\n\nCannot have zero growth\n\n"); exit(1);}
    cons[0]=(float) rho;
    cons[1] = (float) (k-1)/(con->lambda)*exp((*t)*(con->lambda));
    cons[2]= (float) (con->lambda);
    (*t) += (float) bisect(tgrowth, (float) 2*u/k, cons);
  }
}
...
float bisect(bfunc, val, cons) 
float val, *cons, (*bfunc)(float *, float **);
{
...
float tgrowth(var, cons) 
float *var, **cons;
{

  return (float) (*cons)[0]*(*var)+(*cons)[1]*(exp((*cons)[2]*(*var))-1);;
