...
main (int argc, char *argv[]) {
    ...
    seqnames = cmatrix(1, data->nseq+10, 1, MAXNAME+10);
	...
	if ((data->lc != 'C')&&(data->lc != 'L')) nrerror("Must input linear/conversion");
	if (data->lc == 'C') {
	  data->avc=0;
	  while (data->avc <= 0) {
	...
	for (i=1; i<=data->lseq; i++) printf("%3i   %4.3lf\n", i, locs[i]);
	fclose(ifp2);
	...
	if (data->hd ==1 && pnew) { /*Note can have pnew for diploid data, but this has been checked for already*/
		if (!lkf) {
			...
			printf("\n\nInput theta per site (suggest Watterson estimate of %.5f):",(float) data->lseq/(watterson(data->nseq*data->hd)*data->tlseq));
			while (data->th<0.0) scanf("%f", &data->th);
			printf("\n\nMax 4Ner for grid (suggest 100):");
			while(data->rmax<0.0) scanf("%f", &data->rmax);
     ...
     struct data_sum *data;
{
  ...
  if (DEBUG) {
	  printf("\n\nLocs and flocs\n\n");
	for (i=1;i<=data->lseq;i++) printf("%5i %5i\n", locs[i], flocs[i]);
  }
}
     ...
     struct data_sum *data;
{
  ...
  for (i=2, tc[0]=tc[1]=1.0; i<data->nseq*data->hd; i++) {tc[0] += (float) 1/i; tc[1] += (float) 1/(i*i);}
  tc[2] = (float) (data->nseq*data->hd+1)/(3*(data->nseq*data->hd-1));
  tc[3] = (float) 2*((data->nseq*data->hd)*(data->nseq*data->hd)+(data->nseq*data->hd)+3)/(9*(data->nseq*data->hd)*(data->nseq*data->hd-1));
  tc[4] = (float) tc[2]-1/tc[0];
  tc[5] = (float) tc[3]-(data->nseq*data->hd+2)/(tc[0]*data->nseq*data->hd)+tc[1]/(tc[0]*tc[0]);
  tc[6] = (float) tc[4]/tc[0];
  tc[7] = (float) tc[5]/(tc[0]*tc[0]+tc[1]);

  wfp = fopen("window_out.txt", "w");
  ...
  for (win=1;win<=nwin;win++) {
    ...
    if (u!=l) {
      for (cat=1, lkmax=0.0, rhoi=0.0; cat<=data->rce;cat++) {
		rhoe = (float) (cat-1)*(data->rme)/(data->rce-1);  /*Input value of rho for whole window*/
 	  ...
 	  for (i=l,avpwd=0.0;i<=u;i++) {
		  ...
		  avpwd += (float) nmin*(data->nseq*data->hd-nmin);
	  }
	  avpwd *= (float) 2/((data->nseq*data->hd)*(data->nseq*data->hd-1));

      fprintf(wfp,"%10.2f\t%10.2f\t%4i\t%10.5f\t%10.2f\t%10.3f\n", locs[l], locs[u], u-l+1, rhoi/(locs[u]-locs[l]), lkmax-lkg, \
		  (float) (avpwd-(u-l+1)/tc[0])/sqrt(tc[6]*(u-l+1)+tc[7]*(u-l+1)*(u-l)));
    }
    else if (u==l) fprintf(wfp,"%5i - %5i : no data\n", locs[l], locs[u]);
...
struct data_sum *data;
{
	...
	if (ff) {
		...
		printf("\n\nMaximum at 4Ner(region) = %.3f : Lk = %.3lf \n\n",data->rho, data->lkmax);
		ofp = fopen("outfile.txt", "w");
		if (ofp == NULL) {printf("\n\nCannot open outfile\n\n"); exit(1);}
		fprintf(ofp,"Lk surface\n\nTheta = %.5f\n\n", theta);
		fprintf(ofp,"Maximum at 4Ner(region) = %.3f : Lk = %.3lf\n\n", data->rho, data->lkmax);
...
struct site_type **pset;
{
	...
	for (shuff=1; shuff<=NSHUFF;shuff++) {
			...
			for (i=1; i<=data->lseq;i++) pars[i]=1;
				...
				if(lk_calc(pijs, 1, data->lseq, data, &lkrun, locs, (float) (i-1)*(data->rme)/((data->rce)-1), lkmat))
...
double *locs;
{
	...
	if (print_flag) {
		ofp = fopen("rmin.txt", "w");
		fprintf(ofp,"\n\nHudson and Kaplan Rmin matrix\n\n");

		for (i=1;i<data->lseq;i++) {
			fprintf(ofp,"\n%4i: ",i);
			for (j=1;j<i;j++) fprintf(ofp,"\t%3f",(float) min_mat[j][i]/(locs[i]-locs[j]));
			fprintf(ofp,"\t"); j++;
