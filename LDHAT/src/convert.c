...
main (int argc, char *argv[]) {
	...
	long seed = -setseed();
	float fcut=0.0, pwd, sn, ns, ctaj[12], n, d, spwd[2], fgap=1.0;
	double tlseq, *sloc;
	char fname[MAXNAME], bases[6]="0?TCAG", sc[6]="0?0120", c, **seqnames;
	int loc_file = 0;
	...
	for(i = 0; i < argc; i++)
	{
		...
		{ 
			...
			if(strcmp(in_str, "-freqcut") == 0) {fl = 2; fcut = (float) atof(argv[i+1]);}
			if(strcmp(in_str, "-missfreqcut") == 0) fgap = (float) atof(argv[i+1]);
...
/*	printf("\n\nPrint every nth SNP:");
	...
	for (i=l, psite=0;i<=u;i++) {
			...
			else if (fl==2 && na==2 && nmin!=nseq*hd && nmin>((float) nseq*hd*fcut)) nall[i][6]=1;
			else nall[i][6]=0;
			if ( nall[i][1] > ((float) nseq*hd*fgap)) nall[i][6]=0;
	...
	fprintf(loc,"%i %.3lf %c", psite, (inloc ? sloc[u]/*-sloc[l]+2*/ : (float) u-l+1), (inloc ? c : 'L'));

	for (i=1;i<=nseq;i++) if (seq_out_list[i]) {
		fprintf(ofp,"\n>%s\n",seqnames[i]);
		for (j=l,na=0;j<=u;j++) if (nall[j][6]) {
			if (hd==2) fprintf(ofp,"%c",sc[seqs[i][j]]);
	...
	fclose(ofp);
	for (i=l;i<=u;i++) if (nall[i][6]) fprintf(loc,"\n%.3f", (inloc ? sloc[i]/*-sloc[l]+1*/ : (float) i-l+1));
	fclose(loc);

	for (i=1, sn=0, pwd=0, ns=0; i<=lseq; i++) if (nall[i][6]) {
	  sn++; 
	  for (j=2; j<=5; j++) if (nall[i][j]==1) ns++;
	  for (j=2;j<=5;j++) pwd += (float) nall[i][j]*nall[i][j]/(pow(nseq*hd-nall[i][1],2));
	}
	pwd = (float) sn-pwd;
	pwd *= (float) nseq*hd/(nseq*hd-1);
	...
	for (i=1, ctaj[0]=ctaj[1]=0.0; i<nseq*hd; i++) {ctaj[0]+=(float) 1/i; ctaj[1]+= (float) 1/(i*i);}
	n = (float) nseq*hd;
	ctaj[2]=(float) (n+1)/(3*n-3);
	ctaj[3]=(float) 2*(n*n+n+3)/(9*n*(n-1));
	ctaj[4]=ctaj[2]-1/ctaj[0];
	ctaj[5]=(float) ctaj[3]-(n+2)/(n*ctaj[0])+ctaj[1]/(ctaj[0]*ctaj[0]);
	ctaj[6]=ctaj[4]/ctaj[0];
	ctaj[7]=ctaj[5]/(ctaj[0]*ctaj[0]+ctaj[1]);
	ctaj[8]=(float) 2*(n*ctaj[0]-2*(n-1))/((n-1)*(n-2));
	...
	spwd[0]*=(float) 2/(n*(n-1));
	spwd[1]*=(float) 2/(n*(n-1));
	spwd[1]-=(float) spwd[0]*spwd[0];
	...
	printf("\nFu and Li D* statistic = %8.3f", (float) (n/(n-1)*sn-ctaj[0]*ns)/sqrt(ctaj[11]*sn+ctaj[10]*sn*sn));
	if (hd==1) printf("\nVariance PWD           = %8.3f", (float) spwd[1]);
	printf("\n\n");
	...
	for(i = 0; i < argc; i++){
		if(*argv[i] == '-'){ 
			...
			if((strcmp(in_str, "-h") == 0) || (strcmp(in_str, "-?") == 0) || (strcmp(in_str, "-help") == 0)) {
				...
				printf("-freqcut <float>       Minimum Minor Allele Frequency cutoff (between 0 and 1: default=0.0).\n");
				printf("-missfreqcut <file>    Maximum Missing data frequency cutoff (between 0 and 1: default=1.0).\n");
