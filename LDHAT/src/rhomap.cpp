	...
	for(i = 0; i < argc; i++)
	{
		...
		{ 
			...
			if((strcmp(in_str, "-h") == 0) || (strcmp(in_str, "-?") == 0) || (strcmp(in_str, "-help") == 0)) 
			{
				...
				printf(" -fileprefix <string>      Prefix for ALL output files\n");
...
{	// Read in arguments from command line
	...
	if (myMCMC->bpen == -1 ) { printf("\n\nInput block penalty:"); scanf("%f", &myMCMC->bpen); }
	if (myMCMC->hpen == -1 ) { printf("\n\nInput hotspot penalty:"); scanf("%f", &myMCMC->hpen); }
