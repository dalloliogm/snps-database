...
int *revts, **segl;
{
	...
	if (EVENT_PRINT) ofp = fopen("events.txt", "w");
	...
	extern int tree_size;
			...
			for (pos=1;pos<=con->len;pos++) {
				if (list[i]->asite[pos] && list[j]->asite[pos]) {/*Coalescent event at site*/
					...
					tree_ptr[pos][node_pos]->time_below = (float) (tree_ptr[pos][node_pos]->d[0])->time_below+\
					...
					else {list[j]->asite[pos]=NULL; printf("\rSite %5i MRCA : tally = %i",pos, ++tally);}
				}
...
struct results *res;
{
	...
	int mhm, fl=1, fl_asc;
	char bases[5]="NTCAG";
	float tree_len=0, r1, ft, pwd=0, *f, **mutmat, *mm, cump, tt;
	double cf;
	...
	for (site=1, cf=0.0; site<=con->len; site++) {
		...
		if (con->mut) {/*Routines for user defined mutation matrix*/
			...
			if (con->cond) {
				...
				if ((con->inf)&&(nmuts)) nmuts=1;
				for (i=0; i<nmuts; i++) {
				  tt = (float) ((double) tree_len*ran2());
				  j = add_mut(tree[site], &tt);
				  node_mut = tree[site][j];
				  if (DEBUG>1) printf(" : added to node %i: time interval = %.3f - %.3f\n", node_mut->node_num, node_mut->time, (node_mut->a)->time);
...
/*Summary statistics for polymorphism*/
	for (site=1, sn=0, mhm=0; site<=con->len; site++) {
		...
		pwd += (float) fi*((con->nsamp-((int) con->asc==2?NPANEL+1:0))-fi);
	}
	pwd *= (float) 2/((con->nsamp-((int) con->asc==2?NPANEL+1:0))*((con->nsamp-((int) con->asc==2?NPANEL+1:0))-1));

	if (fl) {
	  (*res).sn += (float) sn*exp(cf);
	  (*res).pwd += (float) pwd*exp(cf);
	  if (sn) (*res).mhm += (float) mhm/sn*exp(cf);
...
struct control *con;
{
	...
	*/ 
	...
	fclose(seq);
	loc=fopen("sim.loc","w");
	fprintf(loc,"%i %.3f L", ns, (float) con->rescale);
	for (i=1;i<=con->len;i++) 
		if (seg[i]) 
...
struct control con;
{
	...
	for (i=2, an=1; i<con.nsamp; i++) an += (float) 1/i;
	for (i=1, em=0.0, es=0.0;i<=con.len; i++) {
		em+= (float) con.theta[i];
		es += (float) 1-exp(-con.theta[i]*an);
	}
	...
	printf("Expected               = %.3f\n", (float) es);
	printf("Average PWD            = %.3f\n", (float) res.pwd/res.cf);
	printf("Expected               = %.3f\n", (float) em);
	printf("Average no. rec events = %.3f\n", (float) res.nr/res.cf);
	printf("Expected               = %.3f\n", (float) con.R*an);
	if (con.hyp) printf("Proportion muts HM     = %.3f\n", (float) res.mhm/res.cf);
	printf("ESS                    = %.3f\n", (float) (res.cf)*(res.cf)/(res.cf2));
	printf("\n\n");

	printf("Frequency distribution\n\n");
	for (i=1;i<con.nsamp-((int) con.asc==2?NPANEL+1:0);i++) printf("%3i %8.3f\n",i,res.fdist[i]);
	printf("\n\n");
}
...
char *argv[];
{
	...
	con->theta = vector(1,con->len);
	con->rmap = vector(1,con->len);
	con->theta[1] = -1.0;
	...
	if (con->a>0) 
		for (i=2;i<=con->len;i++) con->theta[i] *= (float) rgamm(con->a);

	if (con->hyp) {
		con->hyp = (int) ((float) (con->len)*(con->phm));
		printf("\nTotal of %i hypermutable sites: ", con->hyp);
		for (i=1;i<=con->hyp;i++) {
			nmut = (int) ((float) ((con->len)-1)*ran2())+2; 
	...
	if (con->cond) {
	  ...
	  ifp = fopen("flocs.txt", "r");
	  for (i=1; i<=con->len; i++) con->slocs[i][1] = con->slocs[i][2]=0;
	  if (ifp) {
	    printf("\n\nReading seg sites from file: flocs\n\n");
	...
	if (con->rm) {
		ifp = fopen("rmap.txt","r");
		if (!ifp) {printf("\n\nCannot open file: rmap\n\n"); exit(1);}
		fscanf(ifp,"%i",&tl);
		if (tl!=con->len) {printf("\nError: recombination for incorrect sequence length\n\n"); exit(1);}
		for (i=1;i<=con->len;i++) {
		  fscanf(ifp,"%i %f", &ss, &(con->rmap[i]));
	...
	if (!(con->rm)){
		for (i=1;i<=con->len;i++) con->rmap[i]=(float) (i-1)*(con->R)/((con->len)-1);
	}


	printf("\n\nParameters: sample %i: Length %i: theta %.4f: R %.4f: Nrun %i : seed = %i\n", \
...
char *argv[];
{
	...
	for(i=1;i<argc;i++) 
	  if ((c=(char *) strchr(argv[i],fl))!=NULL) 
	    switch(*(c+1)) {
	    case 's' : /*User defined seed*/
			if ((i+1)<argc) con->seed = atoi(argv[i+1]);
			else {printf("\nIncorrect flag use: -s seed\n"); exit(1);}
			if (con->seed>0) printf("\nUser-defined seed: %i\n", con->seed);
			else {printf("\nSeed > 0!\n"); exit(1);}
			break;

	    case 'v' : /*Include hypermutable sites: phm=Proportion, rhm=Relative rate */
			con->hyp = 1;
			if ((i+2)<argc) {
	    ...
	    case 'm' : /*User-defined mutation model*/
			printf("\nIncluding non-standard mutation model: in file mut.in\n");
			con->mut=1;
			break;

	    case 'a' : /*Gamma distribution of mutation rates - mean = a*/
	    ...
	    case 'p' : /*Print sequences to file: sites and locs of seg sites to file: locs*/
			printf("\nPrinting sequences (segregating sites only)\n");
			con->print = 1;
			break;

	    case 'g' : /*Population growth*/
	    ...
	    case 'i' : /*Infinite-sites model*/
	                printf("\nUsing infinite site model\n");
	                con->inf = 1;
	                break;

	    case 'c' : /*Condition on mutations occuring at all sites*/
	                printf("\nConditioning on mutations occuring at sites\n");
			con->cond=1;
	                break;

	    case 'r' : /*User defined recombination map*/
			printf("\n\nUser defined recombination map: read from file rmap\n\n");
			con->rm=1;
			break;

	      case 'n': /*SNP ascertainment strategy - simulate an additional n chromosomes*/
		con->asc = atoi(argv[i+1]);
		if (con->asc<2) nrerror("*** Error: ascertainment sample must be at least 2 chromosomes ***");
	      break;

	    case 'x':/*Gene conversion model - input ratio of gene conversion to recombination and average tract length*/
	      con->conv=1;
		if ((i+2)<argc) {
	    ...
	    case 'd':/*Print out sequences as genotypes in 0=00, 1=11, 2=01 format*/
	      con->gt=1;
	      break;

		case 'L' :	/* Rescale output loci file so that loci between 0 and L */
	    ...
	    case 'h': /*Information about options*/
	      printf("\n\nFlags for simulations\n\n");
	      printf("-i     Infinite-sites model for sequences\n\n");
	      printf("-p     Prints segregating sites and locations to files seq and loc \n\n");
	      printf("-c     Condition on mutations at sites\n       no file specificed -> all sites, file flocs.txt->segregating sites\n\n");
	      printf("-f     Analysis restricted to mutations segregating at minimum frequency (specified: e.g. -f 2)\n\n");
	      printf("-g     Population growth with exponential growth parameter (e.g. -g 0.1)\n\n");
	      printf("-b     Population bottleneck with time and strength (e.g. -b 0.1 10)\n\n");
	      printf("-v     Hypermutable sites model: proportion sites  : ratio rates (e.g. -v 0.01 100)\n\n");
	      printf("-a     Gamma distributed rates across sites : shape parameter (e.g. -a 1.0)\n\n");
	      printf("-m     User-defined mutation model in file mut.in\n\n");
	      printf("-r     User-specifed recombination map: requires file rmap.txt\n\n");
	      printf("-s     User-defined seed (e.g. -s 193429012)\n\n");
	      printf("-n     n=2 ascertainment\n\n");
	      printf("-x     Gene conversion model: input ratio conversion to x-over and average tract length (e.g. -x 2 100)\n\n");
...
struct control *con;
{
  ...
  if (!(con->growth)) { /*Constant population size*/
    ...
    (*t) += (double) bisect(tgrowth, (float) 2*u/k, cons);
  }
}
	...
	for(i = 0; i < argc; i++){
		if(*argv[i] == '-'){ 
			...
			if((strcmp(in_str, "-h") == 0) || (strcmp(in_str, "-?") == 0) || (strcmp(in_str, "-help") == 0)) {
				printf("\nfin\n");
				printf("Simulate LDhat format data using Coalescent with recombination\n\n");
				...
				printf("-i                 Infinite-sites model for sequences\n");
				printf("-p                 Prints segregating sites and locations to files sim.seq and sim.loc \n");
				printf("-c                 Condition on mutations at sites\n");
				printf("                     (no file specificed -> all sites, file flocs.txt->segregating sites)\n");
				printf("-f <int>           Analysis restricted to mutations segregating at minimum frequency (specified: e.g. -f 2)\n");
				printf("-g <float>         Population growth with exponential growth parameter (e.g. -g 0.1)\n");
				printf("-b <float> <float> Population bottleneck with time and strength (e.g. -b 0.1 10)\n");
				printf("-v <float> <int>   Hypermutable sites model: proportion sites  : ratio rates (e.g. -v 0.01 100)\n");
				printf("-a <float>         Gamma distributed rates across sites : shape parameter (e.g. -a 1.0)\n");
				printf("-m                 User-defined mutation model in file mut.in\n");
				printf("-r                 User-specifed recombination map: requires file rmap.txt\n");
				printf("-s <int>           User-defined seed (e.g. -s 193429012)\n");
				printf("-n 2               n=2 SNP ascertainment model\n");
				printf("-x <float> <float> Gene conversion model: input ratio conversion to x-over and average tract length (e.g. -x 2 100)\n");
				printf("-d                 Print out sequences as genotypes\n");
				printf("-L <float>         Rescale output loci file so that loci between 0 and L (e.g. -L 6.5)\n");
				printf("-nruns <int>       Number of simulations: NB overwrites sim.seq and sim.loc if -p option used\n");
