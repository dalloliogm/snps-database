...
void allele_count(seqs,nseq,lseq,nall,fl,hd) 
int **seqs,nseq,lseq,**nall, fl, hd;
{
	...
	for (site=1; site<=lseq; site++) {
		for (seq=1; seq<=nseq; seq++) nall[site][seqs[seq][site]]++;
		if (hd==2) {
	...
	for (sites[0]=1; sites[0]<data->lseq; sites[0]++) {
	  printf("\rSite %4i: ",sites[0]);
		for (sites[1]=sites[0]+1; sites[1]<=mini(data->lseq,sites[0]+data->w); sites[1]++) {
			if (!check22(sites[0], sites[1], nall)) pij[sites[0]][sites[1]-sites[0]]=0; 
			else {
			   for (i=0; i<nstate; i++) pt[i]=0;
