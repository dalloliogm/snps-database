...
main (int argc, char *argv[]) {
	...
	data.nseq = nseq; data.hd=1; data.exact=1;
	...
	for (p1=1,ct=0;p1<=i;p1++) for (p2=1;p2<=p1;p2++) for (n11=p2;n11>=0;n11--) {
		ct++;
		pset[ct]->pt[3]=n11;
		pset[ct]->pt[1]=p1-n11;
		pset[ct]->pt[2]=p2-n11;
	...
	print_lks(pset, ns, npmax, lkmat, tcat, theta, rcat, rmax);
...
struct site_type **pset;
{
	...
	fprintf(ofp, "%i %i\n%i ",nseq,npt,tcat);
	for (c1=1; c1<=tcat; c1++) fprintf(ofp, " %f ", theta);
	fprintf(ofp,"\n%i %f\n",rcat, rmax);
