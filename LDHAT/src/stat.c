...
main(int argc, char *argv[]) {
  ...
  arr = vector(1,nline-nmiss);
  ofp = fopen("res.txt", "w");
  mid = (int) ((float) (nline-nmiss)/2);
  l95 = (int) ((float) (nline-nmiss)*0.025+1.0);
  u95 = (int) ((float) (nline-nmiss)*0.975);

  printf("\n\nMid = %i, L95=%i, U95=%i\n\n", mid, l95, u95);


  fprintf(ofp,"Loci\tMean rho\tMedian\tL95\tU95");
  for (i=1;i<=ndat;i++) {
    printf("\rSample %5i", i);
	...
	for(i = 0; i < argc; i++){
		if(*argv[i] == '-'){ 
			...
			if((strcmp(in_str, "-h") == 0) || (strcmp(in_str, "-?") == 0) || (strcmp(in_str, "-help") == 0)) {
				...
				printf("-burn <int>         Number of burn-in iterations (default 0)\n");
